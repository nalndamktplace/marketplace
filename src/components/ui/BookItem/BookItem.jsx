...
const BookItem = ({book,onBuy=()=>{},onOpen=()=>{},layout="GRID", state = 'show', onRead=()=>{}}) => {
	...
	const renderCTA = () => {
		if(book.status === 1)
			switch (state) {
				case 'show':
					return (<div onClick={()=>onOpen()} className='book-item__data__price typo__color--success typo__act utils__d__flex utils__align__center'>{book.price===0?"FREE":<><USDCIcon stroke='currentColor' width={20} height={20}/>{book.price}</>}</div>)
				case 'owned':
